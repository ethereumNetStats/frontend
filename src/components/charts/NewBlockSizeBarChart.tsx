...
export default function NewBlockSizeBarChart(props: {
  latestData: latestBlockData
}) {
  // latestDataからtimestampとtpsを取り出した配列を生成する
  const blockSizeArray: Array<number> = props.latestData.map((blockData) => {
  ...
  let chartData: chartData = [
    {
      name: '',
      data: getNormalizedUnit.ofByteArray(blockSizeArray, 0)
        .data,
    },
  ]
      ...
      labels: {
        ...
        formatter: function (val: string, index: any) {
          return (
            val +
            ` ${
              getNormalizedUnit.ofByteArray(blockSizeArray, 0)
                .unit
            }`
          )
        },
