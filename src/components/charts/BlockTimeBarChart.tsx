...
export default function BlockTimeBarChart(props: {
  latestData: latestBlockData
}) {
  // latestDataからtimestampとtpsを取り出した配列を生成する
  const timestampDiffArray: Array<number> = props.latestData.map((blockData: blockDataWithNewAddressCountAndTps) => {
    return blockData.timestampDiff
  })

  let chartData: chartData = [
    {
      name: '',
      data: getNormalizedUnit.ofTimeDiffArray(timestampDiffArray, 0)
        .data,
    },
  ]
      ...
      labels: {
        ...
        formatter: function (val: string, index: any) {
          return (
            val +
            ` ${
              getNormalizedUnit.ofTimeDiffArray(timestampDiffArray, 0)
                .unit
            }`
          )
        },
