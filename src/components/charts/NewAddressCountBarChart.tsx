...
export default function TpsBarChart(props: { latestData: latestBlockData }) {
  // latestDataからtimestampとtpsを取り出した配列を生成する
  const tpsArray: Array<number> = props.latestData.map(
    (blockData: blockDataWithNewAddressCountAndTps) => {
      return blockData.newAddressCount
    },
  )
  ...
  let chartData: chartData = [
    {
      name: '',
      data: getNormalizedUnit.ofCountArray(tpsArray, 0).data,
    },
  ]
    ...
    yaxis: {
      ...
      labels: {
        ...
        formatter: function (val: string, index: any) {
          return val + ` ${getNormalizedUnit.ofCountArray(tpsArray, 0).unit}`
        },
      },
    },
    grid: {
