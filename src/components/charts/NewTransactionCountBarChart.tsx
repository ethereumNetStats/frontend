...
export default function NewTransactionCountBarChart(props: {
  latestData: latestBlockData
}) {
  // latestDataからtimestampとtpsを取り出した配列を生成する
  const transactionCountArray: Array<number> = props.latestData.map((blockData) => {
    const splitTransactions = blockData.transactions.split(',')
    if (splitTransactions.length === 1) {
      return blockData.transactions.length === 0 ? 0 : 1
    } else {
      return splitTransactions.length
    }
  })
  ...
  let chartData: chartData = [
    {
      name: '',
      data: getNormalizedUnit.ofCountArray(transactionCountArray, 0)
        .data,
    },
  ]
      ...
      labels: {
        ...
        formatter: function (val: string, index: any) {
          return (
            val +
            ` ${
              getNormalizedUnit.ofCountArray(transactionCountArray, 0)
                .unit
            }`
          )
        },
