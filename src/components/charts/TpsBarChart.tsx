...
export default function TpsBarChart(props: {
  latestData: latestBlockData
}) {
  // latestDataからtimestampとtpsを取り出した配列を生成する
  const tpsArray: Array<number> = props.latestData.map((blockData) => {
    return blockData.tps
  })

  let chartData: chartData = [
    {
      name: '',
      data: getNormalizedUnit.ofTpsArray(tpsArray, 0)
        .data,
    },
  ]
      ...
      labels: {
        ...
        formatter: function (val: string, index: any) {
          return (
            val +
            ` ${
              getNormalizedUnit.ofTpsArray(tpsArray, 0)
                .unit
            }`
          )
        },
